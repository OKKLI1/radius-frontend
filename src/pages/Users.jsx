...
export default function Users() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, color: C.text }}>
      ...
      <div style={styles.card}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>{['Usuario', 'Grupo', 'Estado', 'Vencimiento', 'Sesiones max.', 'Acciones'].map((h) => (
      ...
      {modal && (
        <Modal title={modal === 'create' ? 'Nuevo usuario' : `Editar: ${form.username}`}>
          {modal === 'create' && (
            <Field label="Usuario">
              <input style={inputStyle} value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="nombre_usuario" />
            </Field>
          )}
          <Field label={modal === 'create' ? 'Contrasena' : 'Nueva contrasena (dejar vacio = no cambiar)'}>
          ...
          <Field label="Sesiones simultaneas max.">
